 ...
 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * 04/21/11	    ...
 * 10/03/11	    TouchDetectPosition() now returns int16_t for compatibility.
 *****************************************************************************/
...
// Calibration Inset = ( CALIBRATIONINSET / 2 ) % , Range of 0ï¿½20% with 0.5% resolution
...
* Note: none
********************************************************************/
int16_t TouchDetectPosition(void)            //Routine for touch messages including cap and resistive
{
    static long xc,yc;
...
void TouchCalHWGetPoints(void)
{
    ...
    (
        !OutTextXY
            (
                (GetMaxX() - GetTextWidth((XCHAR *)calStr1, (void *) &GOLFontDefault)) >> 1,
    ...
    (
        !OutTextXY
            (
                (GetMaxX() - GetTextWidth((XCHAR *)calStr2, (void *) &GOLFontDefault)) >> 1,
                (GetMaxY() >> 1),
                (XCHAR *)calStr2
            )
    );
    ...
    (
        !OutTextXY
            (
                (GetMaxX() - GetTextWidth((XCHAR *)calStr3, (void *) &GOLFontDefault)) >> 1,
                ((GetMaxY() >> 1) + textHeight),
                (XCHAR *)calStr3
            )
    );
