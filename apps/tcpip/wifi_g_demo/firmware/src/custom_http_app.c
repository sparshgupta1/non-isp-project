...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
    ...
    if (!memcmppgm2ram(filename, "scan.cgi", 8)) {
        ...
        if ((ptr != NULL) && (ptr1 == NULL)) {
            ...
            // scan request
            if (AppConfig.networkType == WF_SOFT_AP) { // SoftAP: display pre-scan results before starting as SoftAP. SoftAP does not scan.
...
/*****************************************************************************
  ...
  Description:
    ...
    The following configurations are possible:
        ...
        ii) Security:
               - None
               - WPA Auto passphrase (passphrase between 8-63 characters)
